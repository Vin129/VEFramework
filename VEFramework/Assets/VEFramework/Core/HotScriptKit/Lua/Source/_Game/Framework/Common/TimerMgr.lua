...
require("Framework/Common/VTimer")
function TimerMgr:ctor()
    self.mTimerList = {};
end

function TimerMgr:AddFrameTimer(callback)
    local frameTimer = require("Framework/Common/FrameTimer").new(callback)
    table.insert(self.mTimerList,frameTimer);
    return frameTimer;
end

function TimerMgr:AddDelayTimer(delayTime,timeCount,callback,firstNotDelay)
    local delayTimer = require("Framework/Common/DelayTimer").new(delayTime,timeCount,callback,firstNotDelay)
